...
class Prism_DaVinciResolve_Integration(object):
    def __init__(self, core, plugin):
        ...
        if platform.system() == "Windows":
            self.examplePath = (
                os.environ["userprofile"] + "\\Documents\\DaVinciResolve\\2019"
            )
            ...
            self.examplePath = os.path.join("/home", userName, "DaVinciResolve", "2019")
            ...
            self.examplePath = (
                "/Users/%s/Library/Preferences/Autodesk/DaVinciResolve/2019" % userName
            )
    ...
    def getExecutable(self):
        execPath = ""
        if platform.system() == "Windows":
            defaultpath = os.path.join(self.getDaVinciResolvePath(), "DaVinciResolve.exe")
    ...
    @err_catcher(name=__name__)
    def getDaVinciResolvePath(self):
        # get executable path
        return ""
    ...
    def addIntegration(self, installPath):
        try:
            ...
            msgStr = (
                "Errors occurred during the installation of the DaVinciResolve integration.\nThe installation is possibly incomplete.\n\n%s\n%s\n%s"
    ...
    def removeIntegration(self, installPath):
        try:
            ...
            msgStr = (
                "Errors occurred during the removal of the DaVinciResolve integration.\n\n%s\n%s\n%s"
                % (str(e), exc_type, exc_tb.tb_lineno)
            )
            msgStr += "\n\nRunning this application as administrator could solve this problem eventually."
    ...
    def installerExecute(self, pluginItem, result):
        try:
            ...
            for i in pluginPaths:
                result["DaVinciResolve integration"] = self.core.integration.addIntegration(self.plugin.pluginName, path=i, quiet=True)
                if result["DaVinciResolve integration"]:
